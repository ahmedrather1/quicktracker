...
import { getAllShows, deleteShow } from "../redux/ShowsListSlice";
...
function ShowsList() {
  ...
  const showState = useSelector((state) => state.showsList);
  const [mappedShows, setMappedShows] = useState([]);

  const [curMedia, setCurMedia] = useState(null);

  useEffect(() => {
    setMappedShows(showState.list);
  }, [showState.list]);
